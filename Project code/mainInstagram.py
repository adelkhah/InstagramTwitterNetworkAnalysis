...
@app.get("/tagshow")
async def homepage(request: Request):
    filename = {'name': 'tagralationinsta.json'}
    return templates.TemplateResponse("tagshow1.html", {"request": request,
                                                       'filename': filename})
...
@app.get("/linkpredict")
async def homepage(request: Request):
    linkp = read_json_file('trendtags/insta_similarity_30.json')
